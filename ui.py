...
from settings import BACKGROUND_COLOR, BASIC_FONT_LARGE, BASIC_FONT_MID
...
class MainWindow(QtWidgets.QMainWindow):
	def __init__(self):
		...
		config_files = [
			f for f in listdir("config_files/") if isfile(join("config_files/", f))
		]
	...
	def start_slicing(self):
		if self.print_file_stl is None:
			self.beep(False)
			return False
	...
	def set_slicer_path(self):
		dialog = QtWidgets.QFileDialog.getOpenFileName(
			self, "Slicerpfad auswählen", filter="EXE (*.exe)"
		)
	...
	def set_stl_files_path(self):
		try:
			dialog = QtWidgets.QFileDialog.getExistingDirectory(
				self, "Modellordnerpfad auswählen"
			)
			path = dialog
			print(path)
	...
	def select_file(self):
		...
		print_file = QtWidgets.QFileDialog.getOpenFileName(
			self,
			"Dateipfad auswählen",
			f"{self.get_stl_model_path()}",
			filter="3D Models (*.stl)",
		)
